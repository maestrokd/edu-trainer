            ...
            <div className="w-full flex flex-col h-full">
                ...
                <div className="flex items-center gap-2 justify-between mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="hidden sm:inline text-xs text-gray-600">Тренажер множення</span>
                        {screen === "play" && (
                            <span className="text-[10px] sm:text-xs text-gray-500">Діапазон: {minVal}–{maxVal} · Точність: {accuracy}%</span>
                        )}
                    </div>
                    {screen === "play" && (
                        <div className="flex items-center gap-2">
                ...
                {screen === "setup" ? (
                    <div className="bg-white/80 backdrop-blur rounded-2xl shadow-lg p-5 sm:p-8 max-w-3xl mx-auto w-full">
                        <p className="text-gray-700 mb-4">
                            Оберіть діапазон множників (мін і макс), наприклад <b>4…9</b>.
                        </p>

                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <label className="flex flex-col">
                                <span className="text-sm text-gray-600 mb-1">Мінімум</span>
                                <select
                                    ...
                                    aria-label="Мінімальне значення діапазону"
                                >
                            ...
                            <label className="flex flex-col">
                                <span className="text-sm text-gray-600 mb-1">Максимум</span>
                                <select
                                    className="rounded-xl border border-gray-300 px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400"
                                    value={maxVal}
                                    onChange={(e) => setMaxVal(parseInt(e.target.value, 10))}
                                    aria-label="Максимальне значення діапазону"
                                >
                                    {Array.from({ length: 11 }, (_, i) => i + 2).map((n) => (
                                        <option key={n} value={n}>
                                            {n}
                            ...
                            <label className="flex flex-col">
                                <span className="text-sm text-gray-600 mb-1">Режим</span>
                                <select
                                    className="rounded-xl border border-gray-300 px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400"
                                    value={mode}
                                    onChange={(e) => setMode(e.target.value as Mode)}
                                    aria-label="Режим тренажера"
                                >
                                    <option value="input">Ввід з клавіатури</option>
                                    <option value="quiz">Квіз (4 варіанти)</option>
                                </select>
                            </label>

                            <div className="flex flex-col sm:col-span-2 lg:col-span-3">
                                <span className="text-sm text-gray-600 mb-1">Вправи</span>
                                <div className="flex flex-wrap items-center gap-3">
                                    <label className="inline-flex items-center gap-2">
                                        <input
                                            ...
                                            aria-label="Включити вправи на множення"
                                        />
                                        <span className="text-sm text-gray-700">Множення</span>
                                    </label>
                                    <label className="inline-flex items-center gap-2">
                                        <input
                                            type="checkbox"
                                            ...
                                            aria-label="Включити вправи на ділення"
                                        />
                                        <span className="text-sm text-gray-700">Ділення</span>
                                    </label>
                                </div>
                            </div>
                        ...
                        <p className="text-xs text-gray-500 mt-3">
                            Діапазон буде автоматично впорядковано (мін ≤ макс) та обрізано до 2…12.
                        </p>
                    </div>
                ) : (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full min-h-0">
                            ...
                            <div className="flex flex-col min-h-0">
                                ...
                                <div className="flex flex-wrap items-center gap-3 mb-6">
                                    <Stat label="Правильних" value={correctCount} />
                                    <Stat label="Неправильних" value={wrongCount} />
                                ...
                                <div className="text-center mb-6">
                                    <div className="text-4xl sm:text-6xl font-semibold tracking-wide text-gray-900 select-none">
                                        {a} <span aria-hidden>{op === "mul" ? "×" : "÷"}</span> <span className="sr-only">{op === "mul" ? "помножити на" : "поділити на"}</span> {b} =
                                    </div>
                                        ...
                                        {mode === "input" ? (
                                            ...
                                            <div className="grid grid-cols-2 gap-3 w-full max-w-md">
                                                {options.map((opt, idx) => (
                                                    <button
                                                        key={`${taskId}-${idx}`}
                                                        onClick={(e) => { (e.currentTarget as HTMLButtonElement).blur(); submit(opt); }}
                                                        ...
                                                        aria-label={`Варіант відповіді ${opt}`}
                                                    >
                                                        {opt}
                                                    </button>
                                                ))}
                                    ...
                                    >
                                        {lastLine && (
                                            <span className="inline-flex items-center gap-2">
                                                ...
                                                {lastCorrect === true && <span role="img" aria-label="Правильно">✅</span>}
                                                {lastCorrect === false && <span role="img" aria-label="Неправильно">❌</span>}
                                            </span>
                                        )}
                                    </div>
                                    {mode === "input" && (
                                        <p className="mt-2 text-xs text-gray-500">Натисніть Enter для відправки відповіді.</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="h-full overflow-auto rounded-xl border border-gray-200 bg-white">
                                <table className="w-full text-left text-sm">
                                    <thead className="sticky top-0 bg-gray-50">
                                        <tr>
                                            <th className="px-4 py-2 font-semibold text-gray-700">Приклад</th>
                                            <th className="px-4 py-2 font-semibold text-gray-700">Відповідь</th>
                                            <th className="px-4 py-2 font-semibold text-gray-700">Результат</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {history.length === 0 ? (
                                            <tr>
                                                <td className="px-4 py-3 text-gray-500" colSpan={3}>
                                                    Історія відповідей зʼявиться тут.
                                                </td>
                                            </tr>
                                        ) : (
                                            history.map((h, idx) => (
                                                    ...
                                                    <td className="px-4 py-2">
                                                        {h.correct ? (
                                                            <span className="inline-flex items-center gap-1 text-green-700">
                                                                <span role="img" aria-label="Правильно">✅</span>
                                                                Правильно
                                                            </span>
                                                        ) : (
                                                            <span className="inline-flex items-center gap-1 text-red-700">
                                                                <span role="img" aria-label="Неправильно">❌</span>
                                                                Неправильно (правильна: {h.op === "mul" ? h.a * h.b : Math.floor(h.a / h.b)})
                                                            </span>
                                                        )}
                                                    </td>
                                                </tr>
