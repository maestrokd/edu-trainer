...
export function DashboardProfileColumn({
  ...
  return (
    <article
      className="flex h-full w-[300px] shrink-0 snap-start flex-col rounded-[32px] border border-white/70 p-4 shadow-sm backdrop-blur-sm sm:w-[320px]"
      style={{
    ...
    >
      <header className="space-y-1">
        <div className="flex items-stretch gap-3">
          ...
          <div className="flex min-w-0 flex-1 flex-col justify-between gap-1">
            <h2 className="truncate text-xl font-semibold leading-tight text-slate-900">{profile.displayName}</h2>

            <div className="flex items-center gap-2">
              <div className="relative h-5 min-w-0 flex-1 overflow-hidden rounded-full bg-white/60">
                <div
                  className="h-full rounded-full transition-all duration-300"
                  style={{ width: `${completionRate}%`, backgroundColor: progressBarColor }}
                />
                <p className="absolute inset-0 flex items-center justify-center text-[11px] font-semibold text-slate-700">
        ...
        <div className="flex items-start gap-2">
          {TOP_SLOT_ORDER.map((slot) => {
            ...
            const ringTrackColor = isDisabled ? "rgba(255, 255, 255, 0.5)" : "rgba(255, 255, 255, 0.72)";
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      ringDegrees > 0
                        ? `conic-gradient(${ringColor} ${ringDegrees}deg, ${ringTrackColor} 0deg)`
                        : ringTrackColor,
                  }}
                >
                  <div
                    className={`flex size-full items-center justify-center rounded-full text-base ${isDisabled ? "bg-white/60" : isOff ? "bg-white/70" : "bg-white/85"}`}
                  >
                    {icon}
                  </div>
                </div>
                <p
                  className={`text-[10px] font-medium capitalize leading-tight ${isDisabled ? "text-slate-500" : isOff ? "text-slate-600" : "text-slate-700"}`}
                >
                  {slotLabel}
                </p>
              </button>
      ...
      <div className="mt-1 min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
        {SECTION_ORDER.map((section) => {
          ...
          return (
            <section key={section} className="space-y-1">
              <h3 className="text-xl font-semibold leading-tight text-slate-600">
        ...
        {profileTasks.length === 0 ? (
          <div className="rounded-2xl border border-white/70 bg-white/55 px-4 py-6 text-center text-sm text-slate-600">
