...
function DashboardDateControls({ isToday, onPreviousDay, onNextDay, onToday, className }: DashboardDateControlsProps) {
  ...
  return (
    <div className={cn("flex shrink-0 items-center gap-1", className)}>
      <button
        className="inline-flex size-9 items-center justify-center rounded-full border border-border bg-background text-foreground shadow-sm transition-colors hover:bg-accent"
      ...
      <button
        className="h-9 rounded-full border border-border bg-background px-3.5 text-sm font-semibold text-foreground shadow-sm transition-colors hover:bg-accent disabled:cursor-not-allowed disabled:opacity-60"
        onClick={onToday}
        disabled={isToday}
      >
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className={cn(
            "inline-flex size-9 items-center justify-center rounded-full border bg-background text-foreground shadow-sm transition-colors hover:bg-accent",
            isAllSelected ? "border-border" : "border-primary/40 bg-primary/10 text-primary",
            className
          )}
      ...
      <DropdownMenuContent
        align="end"
        className="w-[min(92vw,360px)] rounded-2xl border-border bg-popover/95 p-3 text-popover-foreground"
      >
  ...
  return (
    <div className="flex min-w-0 items-center justify-between gap-2">
      <div className="flex min-w-0 items-center gap-2 text-foreground">
        ...
        <span className="hidden shrink-0 items-center gap-1 text-sm font-medium text-muted-foreground min-[561px]:flex">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="start"
            className="w-64 rounded-xl border-border bg-popover/95 p-3 text-popover-foreground"
          >
            <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              {t("familyTask.dashboard.family", "Family")}
            </p>
            <p className="mt-1 break-words text-sm font-semibold text-foreground">{resolvedFamilyName}</p>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
