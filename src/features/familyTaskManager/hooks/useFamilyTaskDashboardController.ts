...
export function useFamilyTaskDashboardController() {
  ...
  useEffect(() => {
    ...
    const timerId = setTimeout(() => {
      setDebouncedSelectedLocalDate(selectedLocalDate);
    }, DASHBOARD_DATE_LOAD_DEBOUNCE_MS);

    return () => clearTimeout(timerId);
  }, [debouncedSelectedLocalDate, selectedLocalDate]);
  ...
  const occurrenceFilters = useMemo(
    () => ({
      from: debouncedSelectedLocalDate,
      to: debouncedSelectedLocalDate,
      profileUuid: effectiveProfileUuid,
    }),
    [debouncedSelectedLocalDate, effectiveProfileUuid]
  );
  ...
  useEffect(() => {
    ...
    track(FamilyTaskAnalyticsEvent.SESSION_STARTED, {
      screen: "dashboard",
      sessionId: `dashboard-${debouncedSelectedLocalDate}-${filterKey}`,
    });
  ...
  const handleComplete = async (task: TaskOccurrenceDto) => {
    ...
    try {
      const updatedTask = await submit(task.uuid, { note: t("familyTask.today.submitted", "Done") });
      if (updatedTask) {
