  ...
  useEffect(() => {
    ...
    eventsApi
      .getAll({
        ...
        to: to.toISOString(),
        assigneeProfileUuid: profileUuid || undefined,
      })
    ...
    <FamilyTaskPageShell>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <article className="rounded-xl border bg-card p-5">
            <p className="text-sm text-muted-foreground">{t("familyTask.calendar.title", "Calendar")}</p>
        ...
        <section className="space-y-3">
          <h2 className="text-xl font-semibold">{t("familyTask.calendar.title", "Calendar")}</h2>
          {eventsLoading && <p className="text-muted-foreground">{t("common.loading", "Loading...")}</p>}
          {!eventsLoading && events.length === 0 && (
            <p className="text-muted-foreground">{t("familyTask.calendar.empty", "No events for today.")}</p>
          )}
          {!eventsLoading && events.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {events.map((event) => (
                <div key={event.uuid} className="rounded-lg border bg-card px-4 py-3">
